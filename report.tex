...
\texttt{TOPIC\_SPECS}. L'utenza può modellare le tematiche, utilizzando
il seguente schema:

\begin{Shaded}
\begin{Highlighting}[]
\ErrorTok{"topic\_title":} \FunctionTok{\{}
...
personalizzabile tramite la voce \texttt{TOPIC\_SELECTOR\_SYSTEM}, la
quale rappresenta il prompt di sistema utilizzato selezionare i
documenti migliori mediante FAISS.
...
\item
  ...
  \tightlist
  \item
    un file \texttt{book.toml} dove definire metadati e configurazioni
    essenziali (eg: directory di output)
  \item
    un file \texttt{SUMMARY.md} dove definire la struttura e la
...
\hypertarget{aspetti-innovazione}{%
\paragraph{Aspetti innovazione}\label{aspetti-innovazione}}
...
Notebook (\texttt{.ipynb}). Per cambiare documentale è necessario
modificare il percorso della variabile di ambiente
\texttt{COOKBOOK\_PATH}. In base ai requisiti progettuali, per questo
manuale il percorso è impostato sul repository OpenAI Cookbook.
...
\tightlist
\item
  Inizialmente sono \emph{file} in formato \texttt{.ipynb}
\item
...
\NormalTok{  subgraph LG[LangGraph – Flusso di generazione]}
\NormalTok{    A[Indicizzazione delle fonti] {-}{-}\textgreater{} B[Recupero documenti rilevanti]}
\NormalTok{    B {-}{-}\textgreater{} C[Selezione documenti]}
\NormalTok{    C {-}{-}\textgreater{} D[Adattamento e generazione sezioni]}
\NormalTok{    D {-}{-}\textgreater{} R[Revisione editoriale]}
\NormalTok{  end}

\NormalTok{  R {-}{-}\textgreater{}|Modifica manuale| R}
\NormalTok{  R {-}{-}\textgreater{}|Approvazione| S[Sezione approvata]}
\NormalTok{  R {-}{-}\textgreater{}|Feedback editoriale| D}

\NormalTok{  subgraph SYS[Sistema di gestione manuale]}
\NormalTok{    S {-}{-}\textgreater{} G[Commit del materiale]}
\NormalTok{  end}

\NormalTok{  subgraph CI[GitHub Actions – Pubblicazione]}
\NormalTok{    G {-}{-}\textgreater{} P[Pubblicazione su GitHub Pages]}
\NormalTok{    P {-}{-}\textgreater{} M[Generazione manuale\textbackslash{}nHTML · PDF · EPUB · TeX · MD]}
\NormalTok{  end}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  \item
    ...
    semantici delle fonti (text-embedding-004)
  \end{itemize}
...
\item
  ...
  \texttt{requirements.txt} tramite il comando
  \texttt{pip\ install\ -r\ requirements.txt}
\item
  \emph{avere un documentale di file} \texttt{.ipynb} all'interno del
  percorso specificato nelle variabili di ambiente
\item
  \emph{eseguire il comando} \texttt{python\ run.py} con l'opzione
  \texttt{-\/-full} per avere un file comprensivo di tutte le sezioni
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \emph{Prompt di selezione} (\texttt{TOPIC\_SELECTOR\_SYSTEM}),
  utilizzati per guidare l'IA nell'identificazione degli estratti più
  pertinenti. Snippet di prompt:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{TOPIC\_SELECTOR\_SYSTEM }\OperatorTok{=} \StringTok{"""}
\StringTok{Sei un assistente che deve selezionare, da una lista di estratti, quelli più rilevanti}
\StringTok{per un manuale aziendale su prompt engineering.}
\StringTok{Scegli solo estratti utili e pratici; scarta parti irrilevanti.}
\StringTok{Restituisci JSON con:}
\StringTok{{-} selected: [\{id, reason\}]}
\StringTok{{-} rejected: [\{id, reason\}]}
\StringTok{"""}
\end{Highlighting}
\end{Shaded}
\item
  \emph{Prompt di generazione/adattamento} (\texttt{ADAPT\_SYSTEM}),
  impiegati per rielaborare i contenuti selezionati, definendone la
  struttura, lo stile ed il livello di dettaglio, in coerenza con gli
  obiettivi ed il contesto dato dall'utente. Snippet di prompt:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{MANUAL\_STYLE\_GUIDE }\OperatorTok{=} \StringTok{"""}
\StringTok{Sei un content strategist senior in un\textquotesingle{}agenzia di comunicazione digitale.}
\StringTok{Obiettivo: creare un manuale d\textquotesingle{}uso dell\textquotesingle{}IA per copywriter e content strategist.}
\StringTok{Stile:}
\StringTok{{-} linguaggio non tecnico, concreto, orientato al lavoro quotidiano}
\StringTok{{-} frasi brevi, punti elenco quando utile}
\StringTok{{-} evita gergo da sviluppo (API, token, embeddings) se non indispensabile}
\StringTok{{-} evidenzia: obiettivo, quando usarlo, come usarlo, esempi, limiti, buone pratiche}
\StringTok{{-} se trovi concetti tecnici nel testo sorgente, riscrivi in modo semplice}
\StringTok{Output: Markdown pulito, modulare, con sezioni coerenti.}
\StringTok{"""}

\NormalTok{ADAPT\_SYSTEM }\OperatorTok{=}\NormalTok{ MANUAL\_STYLE\_GUIDE }\OperatorTok{+} \StringTok{"""}
\StringTok{Regola importante: mantieni riferimenti alla fonte (path file + titolo se presente).}
\StringTok{Non inventare contenuti; se manca un pezzo, segnalalo come \textquotesingle{}Da integrare\textquotesingle{} senza inventare.}
\StringTok{"""}
\end{Highlighting}
\end{Shaded}
\end{enumerate}
...
\item
  ...
  contesto aziendale
\item
  *Miglioramento del livello di accettazione della tecnologia grazie
  alla possibilità di personalizzare i prompt e gestire i contenuti
